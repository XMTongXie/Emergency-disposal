...
![](https://gitee.com/little-magician/picture-resources/raw/master/MarkDown/win-5-1.png)
...
![](https://gitee.com/little-magician/picture-resources/raw/master/MarkDown/win-5-2.png)

进一步使用Log Parser对日志提取数据分析，发现攻击者使用了大量的用户名进行爆破，例如用户名：fxxx，共计进行了17826次口令尝试，攻击者基于“fxxx”这样一个域名信息，构造了一系列的用户名字典进行有针对性进行爆破，如下图：

![](https://gitee.com/little-magician/picture-resources/raw/master/MarkDown/win-5-3.png)
...
![](https://gitee.com/little-magician/picture-resources/raw/master/MarkDown/win-5-4.png)

另外，日志并未记录暴力破解的IP地址，我们可以使用Wireshark对捕获到的流量进行分析，获取到正在进行爆破的IP：

![](https://gitee.com/little-magician/picture-resources/raw/master/MarkDown/win-5-5.png)

通过对近段时间的管理员登录日志进行分析，如下：

![](https://gitee.com/little-magician/picture-resources/raw/master/MarkDown/win-5-6.png)

管理员登录正常，并未发现异常登录时间和异常登录ip，这里的登录类型10，代表远程管理桌面登录。

另外，通过查看FTP站点，发现只有一个测试文件，与站点目录并不在同一个目录下面，进一步验证了FTP暴力破解并未成功。

![](https://gitee.com/little-magician/picture-resources/raw/master/MarkDown/win-5-7.png)
